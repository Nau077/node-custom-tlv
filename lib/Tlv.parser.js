    ...
    Object.keys(data).forEach(key => {
      ...
      if (
        ...
        if (keyArray.length >= 2) {
          ...
          isNest = true;
          helpValue = this.dictionaryReverse[key] + tlvValueLength + tlvValue;
        }
      }

      if (typeof data[key] === "object" && !parent) {
        tlvTag = this.dictionaryReverse[key];
        let r = data[key];
        r;
        tlvValue = this.makeTlv(data[key], { isParent: true });
      ...
      if (typeof data[key] === "object" && parent && parent.isParent) {
        tlvTag = this.dictionaryReverse[key];
