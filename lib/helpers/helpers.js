...
function makeTlvLength(tlvValuelength, customByteLengths) {
  ...
  if (String(tlvValuelength).length == 2) {
    return balanceParityValueLength(tlvValuelength);
  } else if (
    String(tlvValuelength).length > 2 &&
    String(tlvValuelength).length > 5
  ) {
    const customElLength = customByteLengthsArray.find(
      el => el[1].nextByteLengthDefine == 2
    );
    const customStartByte = customElLength.startByte;

    return customStartByte + balanceParityValueLength(tlvValuelength);
  } else {
    const customElLength = customByteLengthsArray.find(
